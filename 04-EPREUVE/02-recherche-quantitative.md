...
# DES RÈGLES SIMPLES AUX MÉTHODES QUANTITATIVES
...
Le quantitatif commence quand une question devient une variable mesurable. La sophistication du modèle doit suivre la quantité d’information disponible et la validation.
...
Un modèle complexe peut améliorer le fit tout en diminuant la robustesse. Une méthode plus simple peut être préférable si elle résiste mieux aux perturbations.
...
Construis un modèle simple puis un modèle légèrement plus complexe. Gèle les données de validation et compare robustesse, interprétabilité et coûts.
...
Tu choisis un modèle pour sa réponse au problème, pas pour son prestige technique.
